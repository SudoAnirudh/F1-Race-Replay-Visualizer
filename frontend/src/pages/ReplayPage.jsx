...
export default function ReplayPage() {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 min-w-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-zinc-900/50 to-zinc-950 overflow-hidden">
      <RaceSelector onSelect={handleSelectRace} />
      
      {data && <StatusBanner data={data} time={time} />}
      
      <main className="flex-1 flex flex-col lg:flex-row gap-3 lg:gap-4 p-3 lg:p-4 min-h-0 min-w-0 overflow-y-auto lg:overflow-y-hidden">
          ...
          <div className="flex-1 min-h-0 relative">
            {session ? (
              ...
              <div className="flex-1 h-full border border-dashed border-zinc-800/40 rounded-2xl flex flex-col items-center justify-center text-zinc-600 bg-zinc-900/20 backdrop-blur-sm animate-fadeIn">
                ...
                <div className="text-lg font-bold tracking-[0.2em] uppercase text-zinc-500">Select a Race</div>
                <div className="text-xs mt-2 opacity-40 font-mono tracking-wider text-zinc-400">Choose a Grand Prix above to start replay</div>
              </div>
            )}
          </div>
